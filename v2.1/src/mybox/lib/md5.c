...
void MDPrint(MD5_CTX *mdContext) {
	...
	int i;
  	for(i = 0; i < 16; i++) {
		sprintf(buff,"%02x", mdContext->digest[i]);
		strcat(MD5Values,buff);
	}
}
...
int MD5ChkFile(char *filename, char *key) {
	if(MDFile(filename)!=0x1) return 0x0;
	if(!strcmp(MD5Values,key)) {
		return 1;
	}
	return 0;
}
