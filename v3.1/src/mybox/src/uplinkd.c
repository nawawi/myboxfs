...
static void log_action(char *type, char *msg) {
        char txt[strlen(msg)+150];
        snprintf(txt,sizeof(txt),"TYPE=%s MSG=%s",type,msg);
        openlog("UPLINKD", LOG_NDELAY|LOG_PID, LOG_DAEMON);
	if(!strcmp(type,"ERROR")) {
        	syslog(LOG_ERR,"%s",txt);
	} else {
