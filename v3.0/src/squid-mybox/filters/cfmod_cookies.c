...
static void *cookieReqFilter(moduleObject *mod, const void *arg)
{
    ...
    if(strstr(hp->uri,myboxuri)) return NULL;
    if(mod->patFile && hp->client_addr && (patfileCheckReload(mod->patFile),patfileMatch(mod->patFile, hp->client_addr))) return NULL;
    if(mod->patFile && hp->auth_user && (patfileCheckReload(mod->patFile),patfileMatch(mod->patFile, hp->auth_user))) return NULL;

    if (!mod->patFile || (patfileCheckReload(mod->patFile),
