...
clientRedirectDone(void *data, char *result)
{
	...
	const char *auth_user, *client_addr;
	...
	if (http->request->auth_user_request) {
        ...
        result = moduleRedirect(result ? result : http->uri, client_addr, auth_user);
    }
#endif
...
clientBuildReplyHeader(clientHttpRequest * http, HttpReply * rep)
{
	...
	const char *auth_user, *content_type, *client_addr;
        client_addr = inet_ntoa(http->request->client_addr);
        if (http->request->auth_user_request) {
             auth_user = authenticateUserRequestUsername(http->request->auth_user_request);
         } else {
...
#if SQUID_FILTERS_DEBUG
	debug(31, 0) ("clientBuildReplyHeader: client=%s content_type=%s auth=%s\n", client_addr,content_type,auth_user);
#endif
        http->filter = moduleCFilterNew(
            httpHeaderGetStr(hdr, HDR_CONTENT_TYPE), http->uri, rep, client_addr, content_type, auth_user);
    }
...
clientSendMoreData(void *data, char *buf, ssize_t size)
{
    ...
    debug(33, 5) ("clientSendMoreData (1): %s, %d bytes\n", http->uri, (int) size);
...
#if SQUID_FILTERS
            if (http->request->flags.filter) {
                HttpHeader *hdr = &rep->header;
                if (httpHeaderHas(hdr, HDR_CONTENT_TYPE)) {
			char *auth_user, *client_addr;
            		client_addr = inet_ntoa(http->request->client_addr);
            		if (http->request->auth_user_request) {
                		auth_user = authenticateUserRequestUsername(http->request->auth_user_request);
           	 	} else {
                    ...
                    const char *typ =  httpHeaderGetStr(hdr, HDR_CONTENT_TYPE);
                    char *x = moduleRejecttype(typ, http->uri,client_addr,auth_user);
                    //debug(33, 8) ("Content-Type: %s%s\n", typ, x?"":" REJECT");
#if SQUID_FILTERS_DEBUG 
                   debug(31, 0) ("Content-Type: %s%s client=%s auth=%s\n", typ, x?"":" REJECT",client_addr,auth_user);
#endif
                    if (!x) {
                        /* KLUGE: call clientWriteComplete so the client
                           socket gets closed. I.e. return empty */
                        clientWriteComplete(fd, NULL, 0, COMM_OK, http);
                        memFree(buf, MEM_CLIENT_SOCK_BUF);
                        return;
                    }
...
#if SQUID_FILTERS_DEBUG
	    debug(31, 0) ("clientSendMoreData: client=%s content_type=%s auth=%s\n",client_addr,content_type,auth_user);
#endif
	    if (moduleCFilter(leakTouch(http->filter),
                              &mb2, buf, body_size, content_type, client_addr, auth_user) < 0) {
...
#if SQUID_FILTERS_DEBUG
	    debug(31, 0) ("clientSendMoreData (2): client=%s content_type=%s auth=%s\n",client_addr,content_type,auth_user);
#endif
	    if (moduleCFilter(leakTouch(http->filter),
                              &mb2, body_buf, body_size, content_type, client_addr, auth_user) < 0) {
                /* filter module told us to abort the request */
                debug(33, 3) ("clientSendMoreData: filter abort\n");
                memBufClean(&mb);
                memBufClean(&mb2);
...
#ifdef SQUID_FILTERS_DEBUG
	    debug(31, 0) ("clientWriteComplete: client=%s content_type=%s auth=%s\n", client_addr,content_type,auth_user);
#endif   
            if (moduleCFilter(leakTouch(http->filter),
                              &mb2, NULL, -1, content_type, client_addr, auth_user) >= 0) {
                if (mb2.size > 0) {
                    comm_write_mbuf(fd, mb2, clientWriteComplete, http);
                    return;
                }
