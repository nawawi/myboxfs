...
#define BBUF_SZ (MAX_URL+32)
	...
	client_addr = inet_ntoa(orig_request->client_addr);
        if (orig_request->auth_user_request) {
             auth_user = authenticateUserRequestUsername(orig_request->auth_user_request);
         } else {
              auth_user = NULL;
         }
        moduleReqHeader(urlCanonical(orig_request), hdr_out,client_addr,auth_user);
    }
#endif
}
