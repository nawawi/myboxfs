...
*/
/*
Ported to mybox 05/02/2007 - nawawi
*/
#include "libmybox.h"
#include <sqlite.h>
...
// the text in memory obtained from xmalloc() and returns a pointer
// to the text.  NULL is returned at end of file, or if xmalloc()
// fails.
...
struct callback_data {
	...
	struct previous_mode_data explainPrev;	// Holds the mode information just before .explain ON
			...
			if( p->cnt++==0 ){
				...
				if( p->showHeader ){
					for(i=0; i<nArg; i++){
						...
						fprintf(p->out,"%-*.*s%s",w,w,"---------------------------------------------------------------------------------------------",i==nArg-1 ? "\n": "  ");
					}
				}
			}
			if( azArg==0 ) break;
