...
function traffic_grpah_value($rrd_source,$datestart,$dateend,$rra_step) {
...
function traffic_graph($type,$dev,$opt=array()) {
	...
	if($type=='update') {
		shell_exec("rrdtool update $rrd_source --template traffic_in N:$value_in");
		shell_exec("rrdtool update $rrd_source --template traffic_out N:$value_out");
		shell_exec("rrdtool tune $rrd_source --minimum traffic_in:0");
		shell_exec("rrdtool tune $rrd_source --minimum traffic_out:0");
	}
	if($type=='save') {
		if(count($opt)==0) return 1;
		$savename=$opt['savename'];
		shell_exec("rrdtool graph $RRD_SAVE_PATH/$savename.png $_cmd_val");
	}
	if($type=='show') {
		return shell_exec("rrdtool graph - $_cmd_val");
	}
}
